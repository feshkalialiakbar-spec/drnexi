...
const InsertOtpForChangePassword = ({
  phone,
  setCode,
}: {
  phone: string
  ...
  const [timeLeft, setTimeLeft] = useState(77)
  ...
  const handleResendOTP = useCallback(() => {
    ...
    const isExpireLastCode = getCookieByName('otpchecktimetosendnewone')
    if (!isExpireLastCode) {
      sendOtpRequest()
    }else{
      toast.error('کمتر از دو دقیقه از آخرین ارسال کد سپری شده است')
      setTimeLeft(20)
      setShowSendOtpButton(false)
    }
  }, [phone])
...
export default InsertOtpForChangePassword
